    ...
    <section id="brands" className="py-20 bg-background">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-bold mb-4 animated-float">
        ...
        <div className="relative">
          ...
          <div className="relative grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8">
            {brands.map((brand, index) => (
              <div
                key={index}
                className="flex items-center justify-center p-6 bg-secondary/30 rounded-lg transform transition-all hover:scale-105 hover:shadow-xl"
              >
                <img src={brand.logo} alt={brand.name} className="h-32 object-contain brand-logo" />
              </div>
            ))}
          </div>
        </div>
